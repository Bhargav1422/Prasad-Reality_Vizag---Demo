...
# -----------------------------
# Feature toggles (you can flip these)
# -----------------------------
ENABLE_HEATMAP   = True
ENABLE_INSTAGRAM = True           # show per-listing "Watch Reel" lazy embed
ENABLE_BOOKING   = True           # slot booking + agent assignment + ICS
ENABLE_LEADS     = True           # lead capture + CSV persistence
ENABLE_SHORTLIST = True
DATA_DIR         = "."            # current folder for demo CSV persistence
...
mock_properties = [
    {
        ...
        # A public reel URL from your handle would go here; for demo we point to handle page.
...
# Initialize session state (shortlist, leads, bookings)
if "properties" not in st.session_state:
    st.session_state.properties = mock_properties
if "shortlist" not in st.session_state:
    st.session_state.shortlist = []
if "leads" not in st.session_state:
    st.session_state.leads = []  # in-memory + persisted CSV
if "bookings" not in st.session_state:
    st.session_state.bookings = []  # in-memory + persisted CSV
if "agents" not in st.session_state:
    st.session_state.agents = mock_agents
...
def generate_ics(summary, description, start_dt, end_dt, location, organizer_email="info@prasadrealityvizag.demo"):
    """
    Simple ICS string (UTC times). For demo we treat input datetimes as naive IST and convert to UTC.
    """
    # Assume IST offset +5:30
...
def assign_agent(locality):
    """
    Simple locality-based assignment: pick first agent covering the locality,
    else fallback to round-robin across full list.
    """
...
st.markdown(
    ...
    <div class="sticky-actions">
      ...
      <span style="margin-left:auto;">
        ...
        https://www.instagram.com/{IG_HANDLE}/Instagram</a>
      </span>
    </div>
    """,
    unsafe_allow_html=True,
...
if show_map and filtered_props:
    ...
    # Weight: price if present else size else 1
...
with col_drawer:
    ...
    if st.session_state.shortlist:
        ...
        lines = [
            f"{p.get('id','')} | {p.get('title','')} | {p.get('locality','')} | {format_price_lakhs(p.get('price_lakhs', 0))} | {p.get('size_sqft', 0)} sqft"
            for p in st.session_state.shortlist
        ]
        ...
        st.markdown(f'{wa_india_all}Share shortlist (India)</a>', unsafe_allow_html=True)
...
if ENABLE_BOOKING and st.session_state.get("show_booking_modal"):
    ...
    if prop:
        ...
        with st.form("booking_form"):
            ...
            duration_min = st.number_input("Duration (minutes)", min_value=15, max_value=180, value=45 if visit_type=="In-person visit" else 30, step=15)
            notes = st.text_area("Notes", placeholder="Any preferences or questions...")
            # Lead basics (so booking isn't orphaned)
            ...
            if submit:
                if not name or not phone:
                    ...
                    wa_customer = whatsapp_link(WA_INDIA_NUM, customer_msg)  # could be agent or main line
        ...
        if submitted:
            if not name or not phone:
                ...
                append_row_csv(LEADS_FILE, lead_row, columns=[
                    "timestamp","name","phone","email","preferred_locality","preferred_type","budget_lakhs","notes","utm_source","utm_medium","utm_campaign","status"
                ])
                st.session_state.leads.append(lead_row)
                st.success("Lead submitted! Our team will reach out shortly.")

    # leads table (demo-visible)
...
st.markdown(
    ...
    <div style="margin-top: 20px; color:#e2e8f0;">
      <strong>Demo notes:</strong> Mocked data      <strong>Demo notes:</strong> Mocked data only, CSV persistence for leads & bookings, no backend.
