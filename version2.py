...
# streamlit_app.py — Public demo with mock data, Instagram-friendly UI, booking + agent assignment + ICS
import os
...
# Branding & CTAs
BRAND_PRIMARY = "#0E57D3"; BRAND_ACCENT = "#FF5A3D"; BRAND_DARK = "#0A2B6D"
BG_DARK       = "#0F172A"; TEXT_LIGHT   = "#F8FAFC"; TEXT_MUTED = "#94A3B8"
...
st.markdown(
    ...
    <style>
      ...
      .brand-header {{
        ...
        padding: 18px 24px; border-radius: 16px; margin-bottom: 18px;
      ...
      @media (max-width: 1000px) {{ .grid {{ grid-template-columns: repeat(2, 1, ); }} }}
      ...
      .pill {{ display:inline-block; padding:4px 8px; border-radius:999px; background:#eef2ff; color:#1e293b; font-weight:700; font-size:12px; border:1px solid #e5e7eb; margin-bottom: 6px; }}
...
mock_properties = [
    {"id":"VRV-101","title":"3BHK Premium — Seethammadhara (2050 sqft, 3rd Fl)",
     ...
     "price_lakhs":205.0,"size_sqft":2050,"bed":3,"bath":3,"lat":17.7449,"lon":83.3120,
     ...
     "tags":["Premium","Vastu","New Listing"],"reel_url":"https://www.instagram.com/reel/C_demo_1/"},
    {"id":"VRV-102","title":"Commercial — Maddilapalem (1200 Sq. Yards)",
     "locality":"Maddilapalem","condition":"New","property_type":"Commercial",
     "price_lakhs":0.0,"size_sqft":10800,"bed":0,"bath":0,"lat":17.7365,"lon":83.3219,
     ...
     "tags":["Investment","High Visibility"],"reel_url":"https://www.instagram.com/reel/C_demo_2/"},
    {"id":"VRV-103","title":"2BHK — Chinnamushidiwada (850 sqft, 2nd Fl)",
     "locality":"Chinnamushidiwada","condition":"New","property_type":"Apartment",
     "price_lakhs":0.0,"size_sqft":850,"bed":2,"bath":2,"lat":17.7780,"lon":83.2350,
     ...
     "tags":["Family Friendly","Good Connectivity"],"reel_url":"https://www.instagram.com/reel/C_demo_3/"},
    {"id":"VRV-104","title":"Open Plot — Bheemili (567 Sq. Yards)",
     "locality":"Bheemili","condition":"New","property_type":"Plot",
     "price_lakhs":113.4,"size_sqft":5103,"bed":0,"bath":0,"lat":17.9050,"lon":83.4520,
     ...
     "tags":["Investment","Peaceful"],"reel_url":"https://www.instagram.com/reel/C_demo_4/"},
]
mock_agents = [
...
def whatsapp_link(number, text): return f"https://wa.me/{number}?text=" + quote_plus(text)
...
def generate_ics(summary, description, start_dt, end_dt, location, organizer_email="info@prasadrealityvizag.demo"):
    ...
    start_utc = start_dt - ist_offset; end_utc = end_dt - ist_offset
...
def assign_agent(locality):
    for ag in st.session_state.agents:
        if locality in ag.get("territories", []): return ag
...
st.markdown(
    ...
    <div class="brand-header">
      <div>
        <h1>Prasad Reality Vizag — Demo Showcase</h1>
        <p>Browse, filter, and shortlist properties. Watch reels, enquire on WhatsApp, capture leads, and book flexible visit slots.</p>
      </div>
    </div>
    """,
    unsafe_allow_html=True,
...
<div class="sticky-actions">
  <span class="pill-mini">🔎 Filters ready</span>
  <span class="pill-mini">📦 {len(st.session_state.properties)} total</span>
  <span class="pill-mini">⭐ Shortlist: {len(st.session_state.shortlist)}</span>
  <span style="margin-left:auto;">
    {wa_hi_india}WhatsApp (India)</a>
    <a class="w.instagram.com/{IG_HANDLE}/Instagram</a>
  </span>
</div>
"""
...
sort_by             = st.sidebar.selectbox("Sort by",
    ["Price (low → high)","Price (high → low)","Size (small → large)","Size (large → small)","Newest Listings"], index=0)
st.sidebar.markdown("---")
...
def apply_filters(data):
    ...
    if selected_condition != "All": df = df[df["condition"] == selected_condition]
    if selected_type != "All": df = df[df["property_type"] == selected_type]
    if search_text:
    ...
    if sort_by == "Price (low → high)": df = df.sort_values(by="price_lakhs", ascending=True)
    elif sort_by == "Price (high → low)": df = df.sort_values(by="price_lakhs", ascending=False)
    elif sort_by == "Size (small → large)": df = df.sort_values(by="size_sqft", ascending=True)
    elif sort_by == "Size (large → small)": df = df.sort_values(by="size_sqft", ascending=False)
    elif sort_by == "Newest Listings": df = df.sort_values(by="condition", ascending=False)
...
def render_property_card(prop: dict):
    ...
    badges_html = "".join([
        f'<span class="badge {"badge-primary" if t in ["New Listing","Premium"] else ""}">{t}</span>' for t in tags
    ])
    ...
    # c3: per-card download optional (disabled)
...
with col_drawer:
    st.markdown("<div class='drawer'><h4>⭐ Shortlist</h4></div>", unsafe_allow_html=True)
    if st.session_state.shortlist:
        for p in st.session_state.shortlist:
            st.markdown(f"<div class='drawer-item'>• <strong>{p.get('id','')}</strong> — {p.get('title','')}</div>", unsafe_allow_html=True)
            st.markdown(f"<div class='drawer-caption'>{p.get('locality','')} • {format_price_lakhs(p.get('price_lakhs', 0))}</div>", unsafe_allow_html=True)
        df_short = pd.DataFrame(st.session_state.shortlist)
        st.download_button("⬇️ Download shortlist (CSV)", data=df_short.to_csv(index=False),
                           file_name=f"shortlist_{datetime.now().strftime('%Y%m%d_%H%M')}.csv", mime="text/csv", use_container_width=True)
        ...
        st.markdown(f'<a class="link-btn primary" href="{wa_india unsafe_allow_html=True)
        st.markdown(f'{wa_us_all}Share shortlist (US)</a>', unsafe_allow_html=True)
    else:
...
if ENABLE_BOOKING and st.session_state.get("show_booking_modal"):
    ...
    if prop:
        ...
        with st.form("booking_form"):
            ...
            duration_min = st.number_input("Duration (minutes)", min_value=15, max_value=180,
                                           value=45 if visit_type=="In-person visit" else 30, step=15)
            ...
            if submit:
                if not name or not phone: st.warning("Please provide at least your name and phone.")
                else:
                    ...
                    location= prop.get("locality","Vizag")
                    ics_text= generate_ics(summary, desc, start_dt_ist, end_dt_ist, location)
                    st.download_button("⬇️ Add to calendar (ICS)", data=ics_text, file_name=f"{booking_id}.ics", mime="text/calendar")

                    customer_msg = f"Booking Confirmed: {prop.get('id')} — {prop.get('title')}\nType: {visit_type}\nWhen (IST): {row['start_dt_ist']} to {row['end_dt_ist']}\nAgent: {agent['name']} ({agent['phone']})"
                    ...
                    st.success(f"Booking scheduled! Assigned agent: {agent['name']} ({agent['phone']})")
                    st.markdown(f'{wa_customer}Notify via WhatsApp (India)</a>', unsafe_allow_html=True)
                    st.markdown(f'{wa_agent}Notify agent (India)</a>', unsafe_allow_html=True)

                    st.session_state["show_booking_modal"] = False
                    st.session_state["booking_target"] = None
                    # st.rerun()  # not required
# Lead capture
if ENABLE_LEADS:
    st.markdown("---")
    ...
    with st.form("lead_capture_form"):
        ...
        if submitted:
            if not name or not phone:
                ...
                append_row_csv(LEADS_FILE, lead_row, columns=[
                    "timestamp","name","phone","email","preferred_locality","preferred_type",
                    "budget_lakhs","notes","utm_source","utm_medium","utm_campaign","status"])
                st.session_state.leads.append(lead_row)
                st.success("Lead submitted! Our team will reach out shortly.")

    if st.session_state.leads:
        df_leads = pd.DataFrame(st.session_state.leads)
        st.dataframe(df_leads, use_container_width=True)
        st.download_button("⬇️ Download leads (CSV)", data=df_leads.to_csv(index=False),
...
if ENABLE_INSTAGRAM:
    ...
    if insta_url and insta_url.startswith("https://www.instagram.com/reel/"):
        ...
        st.markdown(
            f'https://www.instagram.com/{IG_HANDLE}/Open Instagram Profile</a>',
            unsafe_allow_html=True,
        )
...
st.markdown(
    ...
    <div style="margin-top: 20px; color:#e2e8f0;">
      <strong>Demo notes:</strong> Mocked data only; CSV persistence for leads & bookings; no backend.
      <br>Contact via WhatsApp      <br>Contact via WhatsApp India (+91 6309729493) or US (+1 786 420 9015).
    </div>
    """,
    unsafe_allow_html=True,
    )
